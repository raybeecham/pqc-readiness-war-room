...
__name(isValidPublicHostname, "isValidPublicHostname");
...
__name(scoreTargetSignals, "scoreTargetSignals");
...
__name(confidenceFromTarget, "confidenceFromTarget");
function signalLine(observed, text) {
  return observed ? `<div class="signal observed">\u2713 ${text}</div>` : `<div class="signal missing">\u2715 ${text}</div>`;
}
__name(signalLine, "signalLine");
var src_default = {
  async fetch(request) {
    const cf = request.cf || {};
    ...
    const cipherIsAead = cipher.includes("GCM") || cipher.includes("CHACHA") || cipher.includes("POLY1305") || cipher.includes("AEAD");
    ...
    if (cipherIsAead) {
      ...
      findings.push({
        ...
        status: "PASS",
        detail: "Observed cipher appears to use modern authenticated encryption."
      });
    } else {
      findings.push({
    ...
    if (tlsVersion !== "TLSv1.0" && tlsVersion !== "TLSv1.1" && tlsVersion !== "TLSv1.2") {
    ...
    const visibilityImpact = usesHttp3 ? "HTTP/3 indicates QUIC-based transport. This can reduce traditional TCP-centric inspection assumptions." : usesHttp2 ? "HTTP/2 over TLS provides a familiar TCP-based enterprise visibility posture." : "Protocol visibility is unclear. Validate from browser, proxy, and edge logs.";
      ...
      if (!isValidPublicHostname(target)) {
        ...
        try {
          ...
          const signals = {
            ...
            limited: response.status === 401 || response.status === 403 || response.status === 429 || response.status === 503,
    ...
    const recommendation = tlsModernizationScore >= 75 ? "Strong TLS modernization posture. Next step: inventory cryptographic dependencies, validate vendor PQC roadmaps, and map visibility requirements." : tlsModernizationScore >= 55 ? "Moderate TLS modernization posture. Next step: confirm TLS 1.3 coverage, document visibility gaps, and begin PQC readiness planning." : "Weak TLS modernization posture. Next step: prioritize TLS modernization, remove legacy protocol dependencies, and establish crypto-agility governance.";
    console.log(
      "PQC_READINESS_SCAN",
      JSON.stringify({
    ...
    const findingRows = findings.map((f) => {
      const cls = f.status === "PASS" ? "pass" : f.status === "FAIL" ? "fail" : f.status === "WARN" ? "warn" : "info";
      return `
    ...
    const targetHtml = targetAssessment ? targetAssessment.reachable ? `
          <div class="metric ${targetAssessment.score >= 75 ? "green" : targetAssessment.score >= 50 ? "yellow" : "red"}">${targetAssessment.score}/100</div>
          <div class="bar">
            <div class="${targetAssessment.score >= 75 ? "fill" : targetAssessment.score >= 50 ? "fill-yellow" : "fill-red"}" style="width:${targetAssessment.score}%"></div>
          </div>
          ...
          ${targetAssessment.limited ? `<div class="warning-box">
...
// .wrangler/tmp/bundle-GwFhm3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-GwFhm3/middleware-loader.entry.ts
